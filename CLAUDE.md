...
- **Not yet done**: joining `show_ratings` into `mlb_players_master.json` via `build_mlb_match.py`/`export_mlb_master.py` — this round only covers getting the data into the DB (Part 3's actual scope), the master-file join is a natural next step, not yet started.
...
- ⬜ MLB ratings data collected (`show_ratings`, 1346 rows, all 30 teams, via a ScraperAPI proxy rebuild — see theshowratings.com Scraper section) but not yet joined into `mlb_players_master.json`; `build_mlb_match.py`/`export_mlb_master.py` need a ratings join added, same shape as the roster/stats joins already there.
