...
- **Real bug found this session, not yet fixed**: when `fetch_with_retry()` exhausts its blocked-retry budget, it logs and falls through without raising or returning a response, so the caller's `BeautifulSoup(html, ...)` call crashes with `TypeError: Incoming markup is of an invalid type: None` instead of failing cleanly. `mlb/scripts/scrape_ratings.py`'s `BlockedError` pattern (raise a typed exception, catch it once in `main`, report cleanly, still write whatever partial data exists) doesn't exist here — worth porting over next time this script is touched.
...
- NBA: `scrape_2kratings.py` re-confirmed blocked this session (1/30 teams, then a crash — see 2K Ratings Scraper section above for the `fetch_with_retry` None-return bug found alongside it). Not resolved.
- MLB: `scrape_ratings.py` (theshowratings.com) blocked a third consecutive session, immediately again (0/30 teams) — a Playwright headless-Chromium prototype hit the identical 403, real evidence this block isn't TLS-fingerprint-specific (see theshowratings.com Scraper section above). Not resolved, and `curl_cffi` alone is no longer confidently "the fix" for this one.
- NHL: `scrape_ratings.py` (nhlratings.net) re-attempted this session with `curl_cffi` + 8-15s pacing — 1/32 teams (ANA) before block. Script and matching logic both confirmed working (91.9% match rate on the one completed team); the block itself is what's unresolved.
