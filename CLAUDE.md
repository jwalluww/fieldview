...
- **Fan-made**: `nhlratings.net` — structure fully confirmed (see below), but **now blocked mid-session, not clean**. Plain `requests.get()` returned clean 200s (full player table already in raw HTML, WordPress/Yoast, `cf-cache-status: HIT`) for the first several requests across the probe session, then a follow-up request to a team page that had never been hit before came back a real Cloudflare 403 (`Server: cloudflare`, real `cf-ray`, `cf-cache-status: BYPASS` — freshly computed, not a cached page). Same rate/volume-triggered-cooldown signature already seen on 2kratings.com and theshowratings.com, not a first-request rejection — points at request volume from repeated probing, not TLS fingerprinting, so `nhl/scripts/scrape_ratings.py` deliberately still uses plain `requests`, not `curl_cffi`. **Standing rule applies: not retried same-day.** Structure confirmed before the block hit: 32 per-team pages (`/teams/{team-slug}`), each with a main roster table (TOR: 29 rows) plus a small second table for IR/non-roster players (TOR: 1 row), plus separate `/lists/{position}` pages (centers, defensemen, goalies, etc.) and individual player pages at a bare name-slug URL (`/william-nylander`, no numeric ID in the URL itself).
...
- **Both ratings scripts' per-team delay bumped to 8-15s** (`nhl/scripts/scrape_ratings.py` from 1.5s, `mlb/scripts/scrape_ratings.py` from 2.5s) — neither runs more than roughly weekly and ratings barely move day to day, so there's no reason to hurry. Documented in both scripts' comments as *not* expected to fix a TLS-fingerprint block by itself (that needs `curl_cffi`, confirmed elsewhere in this project) but as real insurance against the separate, request-volume-triggered cooldown that's the more likely explanation for what hit `nhlratings.net` mid-probe this session. Neither script was run again this round — both still mid-cooldown.
...
- MLB: `scrape_ratings.py` (theshowratings.com) blocked pending a genuine cooldown — re-confirmed blocked this session (0/30 teams, 403 on the first team), script is correct and ready, see NHL Ratings sources section above for the re-test. Not resolved.
- NHL: `scrape_ratings.py` (nhlratings.net) blocked mid-probe this session — script is correct and ready (name+team fuzzy matching, no clean ID join available), see NHL Ratings sources section above. Not resolved.
