...
- **Proxy rebuild (Part 3) designed, explicitly not started**: the plan is to route requests through a proxy endpoint (e.g. ScraperAPI-style, `http://api.scraperapi.com?api_key={key}&url={target}`) read from a `SCRAPERAPI_KEY` environment variable, never hardcoded/committed, single-page-tested before a full 30-team pass. Checked live this session: **no `SCRAPERAPI_KEY` is set**. Per explicit instruction, this only starts once a real key exists — nothing to report yet, and no proxy code has been written into `scrape_ratings.py`.
...
- MLB: `scrape_ratings.py` (theshowratings.com) blocked a third consecutive session, immediately again (0/30 teams) — a Playwright headless-Chromium prototype hit the identical 403, real evidence this block isn't TLS-fingerprint-specific (see theshowratings.com Scraper section above). Not resolved, and `curl_cffi` alone is no longer confidently "the fix" for this one. A proxy-routed rebuild (Part 3, ScraperAPI-style) is designed but explicitly not started — no `SCRAPERAPI_KEY` exists yet (checked live this session, unset). Nothing to test until a key exists.
